...
API.version = '1.2'
const API_1484_11 = new apiclass()
...
export function apiclass () {
  ...
  function _LMSInitialize (val) {
    ...
    this._sessionid = 00// SCOClient.sessionid;
    this._userid = 00 // SCOClient.userid;
    this._coreid = 00// SCOClient.coreid;
    this._scorm_course_id = 00// SCOClient.scorm_course_id;
    this._sco_identifier = 00// SCOClient.sco_identifier;
    // LMSInfo object carries arguments to the server and back
    let lmsInfo = JSON.stringify(createLMSInfo(this._sessionid, this._userid, this._coreid, this._scorm_course_id, this._sco_identifier))
    WriteToDebug('LMSInitialize: ' + lmsInfo)
    const that = this // get reference to current API instance
    console.log()
    this.initialized = true
    lmsInfo = '{"coreId": "5",  "dataItem": "null", "dataValue": "null", "errorCode": "0","errorDiagnostic":"null","errorString":"null","returnValue":"true","scoIdentifier":"default","scormCourseId":"9","sessionId":"17","userId":"5454s"}'
    /*
        $.ajax({
            type: "POST",
            url: "localhost:5001/api/LMSInitialize",
            data: lmsInfo,
            dataType: "json",
  ...
  function _LMSFinish (val) {
    ...
    // LMSInfo object carries arguments to the server and back
    let lmsInfo = JSON.stringify(createLMSInfo(this._sessionid, this._userid, this._coreid, this._scorm_course_id, this._sco_identifier))
    WriteToDebug('LMSFinish: ' + JSON.stringify({ lmsInfo }))
    const that = this // get reference to current instance
    $.ajax({
      type: 'POST',
      url: '/api/LMSFinish',
      data: lmsInfo,
  ...
  function _LMSGetValue (name) {
    ...
    let lmsInfo = JSON.stringify(createLMSInfo(this._sessionid, this._userid, this._coreid, this._scorm_course_id, this._sco_identifier, name))
    WriteToDebug('GETVALUE: ' + JSON.stringify({ lmsInfo }))
  ...
  function _LMSSetValue (name, value) {
    ...
    let lmsInfo = JSON.stringify(createLMSInfo(this._sessionid, this._userid, this._coreid, this._scorm_course_id, this._sco_identifier, name, value))
    WriteToDebug('SETVALUE: ' + JSON.stringify({ lmsInfo }))
    let returnValue = ''
    const that = this // get reference to current API instance
    $.ajax({
      type: 'POST',
 ...
 *
*********************************************************************************/
export function createLMSInfo (Sessionid, user_id, core_id, SCORM_course_id, SCO_identifier, DataItem, DataValue, ErrorCode, ErrorString, ErrorDiagnostic, ReturnValue) {
  const o = new Object()
...
**
*********************************************************************************/
export function WriteToDebug (strInfo) {
  if (blnDebug) {
    const dtm = new Date()
    let strLine
...
// public
export function DisplayError (strMessage) {
  let blnShowDebug
